...
| `AsyncCsvLogger.kt` (`app/src/main/java/com/vag/vcdsandroid/logging/AsyncCsvLogger.kt`) | High-speed, non-blocking asynchronous CSV recorder (buffered channel + dedicated I/O coroutine). Writes synchronized turbo pairs, pre-log SESSION_SNAPSHOT metadata, and raw events. |
