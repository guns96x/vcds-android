...
class AndroidUsbProbeTest {
    ...
    @Test
    fun `classifyProfile identifies generic FTDI KKL 6001`() {
        val (profile, hypothesis) = AndroidUsbProbe.classifyProfile(0x0403, 0x6001)
        assertEquals(HardwareProfile.GENERIC_FTDI_KKL, profile)
        assertTrue(hypothesis.contains("Direct K-Line pass-through"))
    }

    @Test
    fun `classifyProfile identifies CH340 KKL adapters`() {
        val (profile, _) = AndroidUsbProbe.classifyProfile(0x1A86, 0x7523)
        assertEquals(HardwareProfile.CH34X_KKL, profile)
    }

    @Test
    fun `classifyProfile identifies CP210x KKL adapters`() {
        val (profile, _) = AndroidUsbProbe.classifyProfile(0x10C4, 0xEA60)
        assertEquals(HardwareProfile.CP210X_KKL, profile)
    }

    @Test
    fun `classifyProfile identifies PL2303 KKL adapters`() {
        val (profile, _) = AndroidUsbProbe.classifyProfile(0x067B, 0x2303)
        assertEquals(HardwareProfile.PL2303_KKL, profile)
    }
